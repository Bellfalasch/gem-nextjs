...
export async function getStaticProps(context: Context) {
  ...
  const canNotRender =
    meta && !meta.canRender && meta.renderMode !== RENDER_MODE.EDIT;
...
export async function recursiveFetchChildren(
...
function collectPageComponentUrls(
  regions: RegionTree,
  contentPath: string
): Item[] {
  let items: Item[] = [];
...
async function doRecursiveFetch(
  contentApiUrl: string,
  path: string,
  maxLevel = 0,
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  return result?.guillotine?.getChildren.reduce(
    ...
    async (prevPromise: Promise<Item[]>, child: any) => {
      ...
      if (child.pageAsJson?.regions) {
        const compUrls = collectPageComponentUrls(
          child.pageAsJson?.regions,
          contentPath
        );
        if (compUrls.length) {
